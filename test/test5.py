        ...
        with connection.cursor() as cursor:
            for i in range(100000):
                try:
                    ...
                    print(f"user_id: {user_id}, phone: {phone}, email: {email}, password: {password}, avatar_key: {avatar_key}, login_status: {login_status}, account_status: {account_status}, login_datetime: {login_datetime}, logout_datetime: {logout_datetime}, user_status: {user_status}, operator: {operator}")

                    # 插入单条数据
                    sql = """
                    INSERT INTO user (user_id, phone, email, password, avatar_key, login_status, account_status, login_datetime, logout_datetime, user_status, operator, create_datetime, update_datetime, delete_datetime)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
                    """
                    cursor.execute(sql, (
                        user_id, phone, email, password, avatar_key, login_status, account_status, login_datetime,
